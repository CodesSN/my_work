...
export class EditComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.filteredOptions = this.trucksForm?.get('barber')?.valueChanges.pipe(
      startWith(''),
    ...
    await this.employeeService.getAllEmployees().subscribe(async (data) => {
      data.body.forEach(async (e) => {
        if ((e as Employee).name === this.trucksForm?.value.barber) {
          ...
          console.log(
            'd:',
            parseInt(localStorage.getItem('id_barber') as string)
          );
        }
      });
    });
    return parseInt(localStorage.getItem('id_barber') as string);
  }
  async submit() {
